...
import { LineOutlined, FolderOpenOutlined } from '@ant-design/icons';
import { validateConfigurableComponentSettings, useFormData, useGlobalState, getLayoutStyle, DesignerToolbarSettings, ShaIcon, useSheshaApplication } from '@shesha-io/reactjs';
...
const DividerComponent = {
    ...
    Factory: ({ model }) => {
        ...
        return (jsx(Divider, { style: getLayoutStyle(model, { data, globalState }), ...props }));
    },
...
const getSettings = (data) => new DesignerToolbarSettings(data)
    .addSectionSeparator({
    id: nanoid(),
...
const EmptyComponent = {
    ...
    isInput: false,
    name: 'Empty',
    icon: jsx(FolderOpenOutlined, {}),
    Factory: ({ model }) => {
        const { description, image } = model;
        if (model.hidden)
            return null;
        return (jsx(Empty, { description: description, image: image ? jsx(ShaIcon, { iconName: image, size: 45 }) : Empty.PRESENTED_IMAGE_DEFAULT }));
    },
...
export { ApplicationPlugin, formDesignerComponents };
