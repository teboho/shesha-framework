...
import { useConfigurableAction, useConfigurableActionDispatcher, useHttpClient, useTheme } from '@/providers';
...
const DataListControl: FC<IDataListWithDataSourceProps> = (props) => {
  ...
  const data = useDeepCompareMemo(() => {
    // If we have no data OR no modelType (misconfigured), provide some sample user data for preview
    if (!tableData || tableData.length === 0 || !modelType) {
      return [
        {
          id: "sample-1",
  ...
  // Check if we're showing mock data (when no tableData or no modelType)
  const isShowingMockData = !tableData || tableData.length === 0 || !modelType;
  ...
  const registerFallbackProperties = useCallback(() => {
  ...
  const preloadFormTemplate = useCallback(async () => {
    ...
    try {
      ...
      if (formToLoad) {
        // Load form by ID
        try {
          const formConfig = await getForm({ formId: formToLoad, skipCache: false });
          if (formConfig) {
        ...
        try {
          ...
          const formConfig = await getForm({ formId, skipCache: false });
          if (formConfig) {
            const extractedFields = extractFieldsFromFormConfig(formConfig);
            if (extractedFields.length > 0) {
              await registerExtractedFields(extractedFields);
  ...
  }, [props.formSelectionMode, props.formId, props.formType, modelType, hasRegisteredFields, registerExtractedFields, registerSmartDisplayProperties, registerFallbackProperties, getForm, getEntityFormId, isDesignMode, props.id]);

  // Handle fields extracted from form template (for runtime extraction)
  const handleFieldsExtracted = useCallback((fields: string[]) => {
  ...
  const dataListProps = useMemo(() => {
    // Determine context scenario based on dataSource and modelType
    let contextScenario: 'outside-datacontext' | 'no-entitytype' | 'no-form' = 'no-form';

    // Check if we're outside DataContext (using the flag from mockDataSource)
    if (dataSource && (dataSource as any)._isOutsideDataContext) {
      contextScenario = 'outside-datacontext';
    } else if (!modelType) {
      contextScenario = 'no-entitytype';
    }
    ...
    return baseProps;
  }, [props, isFormMissing, canAddInline, canEditInline, canDeleteInline, data, selectedRow, selectedRows, handleFieldsExtracted, dataSource, modelType]);
    ...
    >
      {isShowingMockData ? (
        <>
          <style>
            {`
              .ant-popover.sha-data-list-control-hint-popover .ant-popover-inner {
                background-color: rgb(214, 214, 214) !important;
                border-radius: 8px !important;
              }
              .ant-popover.sha-data-list-control-hint-popover .ant-popover-arrow::before,
              .ant-popover.sha-data-list-control-hint-popover .ant-popover-arrow::after {
                background-color: rgb(214, 214, 214) !important;
              }
            `}
          </style>
          <div style={{ position: 'relative' }}>
            <Tooltip title="Data List is showing sample data due to missing configuration.">
              <div style={{
                opacity: 0.8,
                filter: 'grayscale(50%)',
                position: 'relative',
            ...
            >
              <Popover
                ...
                rootClassName="sha-data-list-control-hint-popover"
                content={(
                  <p>
                    {!modelType ? (
                      <>
                        The Data Context has no Entity Type configured.<br />
                        <br />
                        Please configure the Entity Type in the Data Context settings.
                      </>
                    ) : (
                      <>
                        The Data List item template is not configured.<br />
                        <br />
                        Configure a Form ID for the data list item template<br />
                        (&quot;Form&quot; property in the Data section).
                      </>
                    )}
                    <br />
              ...
              >
                <InfoCircleOutlined style={{ color: theme?.application?.warningColor || '#faad14', cursor: 'help', fontSize: '16px' }} />
              </Popover>
            </div>
          </div>
        </>
